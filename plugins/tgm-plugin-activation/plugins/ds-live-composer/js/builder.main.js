 ...
 * WYSIWYG Editor
 *
 * - dslc_editable_content_gen_code ( generate and apply content )
 */
...
/** 
 * Scroller ( For the modules and templatest listing )
 *
 * - dslc_scroller_init ( Initialize the scroller )
 * - dslc_scroller_go_to ( Scroll to specific item )
 * - dslc_scroller_prev ( Scroll to previous item )
 * - dslc_scroller_next ( Scroll to next item )
 */
	...
	function dslc_scroller_init() {
		...
		if ( scrollers.length ) {
			...
			scrollers.each(function(){
				...
				var scrollerInnerOffset = scrollerInner.position();
				var scrollerContent = jQuery('.dslca-section-scroller-content', scroller);
				var scrollerItems = jQuery('.dslca-scroller-item:visible', scroller);

				var startingWidth = 0;
				var scrollerWidth = scroller.width() + scrollerInnerOffset.left * -1;
				var scrollerContentWidth = scrollerContent.width();
	...
	function dslc_scroller_go_to( scrollerItemIndex, scroller ) {
		...
		var scrollerInner = jQuery('.dslca-section-scroller-inner', scroller);
		var scrollerContent = jQuery('.dslca-section-scroller-content', scroller);
		var scrollerItems = jQuery('.dslca-scroller-item', scroller);
		var scrollerItem = jQuery('.dslca-scroller-item:eq(' + scrollerItemIndex + ')', scroller);

		if ( scrollerItem.length ) {

			var scrollerWidth = scroller.width();
			var scrollerContentWidth = scrollerContent.width();
			var scrollerItemOffset = scrollerItem.position();
	...
	function dslc_scroller_prev( scroller ) {
		...
		scrollerNew = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')').prevAll('.dslca-scroller-item:visible').eq(1).index(),
		scrollerNewAlt = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')').prevAll('.dslca-scroller-item:visible').eq(0).index();
	...
	function dslc_scroller_next( scroller ) {
		...
		var scrollerCurr = scroller.data('current'),
		scrollerNew = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')').nextAll('.dslca-scroller-item:visible').eq(1).index(),
		scrollerNewAlt = scroller.find('.dslca-scroller-item:eq(' + scrollerCurr +  ')').nextAll('.dslca-scroller-item:visible').eq(0).index();
 ...
 * Composer
 *
 * - dslc_generate_code ( Generates the front-end code for added modules )
 * - dslc_generate_section_code ( Generates the code for a specific section )
 * - dslc_option_changed ( Shows the save )
 * - dslc_save_composer ( Saves the composer code )
 * - dslc_show_section ( Shows a section of composer )
 * - dslc_show_modal ( Shows a modal )
 * - dslc_hide_modal ( Hides a modal )
 * - dslc_filter_origin ( Filters templates and modules based on origin )
 */
	...
	function dslc_option_changed() {
		...
		if ( dslcDebug ) console.log( 'dslc_option_changed' );
	...
	function dslc_save_composer() {
		...
		var contentForSearch = jQuery('#dslca-content-for-search').val();
		var postID = jQuery('.dslca-container').data('post-id');
	...
	function dslc_show_section( section ) {
		...
		var newColor = jQuery(section).data('bg');

		jQuery('.dslca-container').animate({ bottom: -500 }, 200, function(){
			jQuery(this).animate({ bottom : 0 }, 300 );
		});

		jQuery('.dslca-sections').animate({ backgroundColor : newColor }, 300);

		jQuery('.dslca-section:visible').animate({ opacity : 0 }, 150, function(){

			jQuery('.dslca-section').hide();
			jQuery(section).show().animate({ opacity : 1 }, 150, function(){

				// Initiate scrollbar
				dslc_init_options_scrollbar();
				if ( section == '.dslca-modules-section-edit' ) { dslc_edit_modules_section_scroller(); }
				dslc_init_options_slider();
				dslc_show_module_options_tab();
				dslc_scroller_init();
	...
	function dslc_show_modal( hook, modal ) {
		...
		var diff = modal.outerWidth() / 2 - hook.outerWidth() / 2;
		var offset = position.left - diff;
 ...
 * Modules
 *
 * - dslc_module_dragdrop_init ( Initiates drag and drop on modules and module areas )
 * - dslc_get_module_output ( Gets module front-end output  )
 * - dslc_display_module_options ( Display options for a module )
 * - dslc_delete_module ( Deletes a module )
 * - dslc_preview_change ( Preview module when option changed )
 * - dslc_module_width_set ( Set a specific width of module )
 * - dslc_copy_module ( Copies module )
 * - dslc_cancel_changes ( Back to options before changing )
 * - dslc_confirm_changes ( Leave current changes as final )
 * - dslc_init_colorpicker ( Initiates the colorpicker )
 */

	function dslc_module_dragdrop_init() {

		if ( dslcDebug ) console.log( 'dslc_module_dragdrop_init' );
		...
		jQuery( '.dslc-modules-section' ).droppable({
			drop: function( event, ui ) {
				...
				if ( moduleID == 'DSLC_M_A' ) { 
					dslc_add_modules_area( modulesSection );
				}
			}
		});
		...
		jQuery( '.dslc-modules-area' ).droppable({
			...
			drop: function( event, ui ) {
				...
				if ( moduleID == 'DSLC_M_A' || jQuery('body').hasClass('dslca-module-drop-in-progress') ) { 
					...
					// Load Output
					dslc_get_module_output( moduleID, function( response ){
	...
	function dslc_get_module_output( dslcModuleID, callback ) {

		if ( dslcDebug ) console.log( 'dslc_get_module_output' );
	...
	function dslc_display_module_options( dslcModuleID ) {

		if ( dslcDebug ) console.log( 'dslc_display_module_options' );
		...
		jQuery.post(
			...
			function( response ) {
				...
				// Add the options
				jQuery('.dslca-module-edit-options').html( response.output );
				...
				// Initniate Colorpicker
				dslc_init_colorpicker();
				dslc_init_options_slider();
				...
				// Hide/Show Tabs
				dslc_module_edit_options_hideshow_tabs();
	...
	function dslc_filter_module_options( dslcOptionsType ) {

		if ( dslcDebug ) console.log( 'dslc_filter_module_options' );

		jQuery('.dslca-module-edit-option').hide();
		jQuery('.dslca-module-edit-option[data-section="' + dslcOptionsType + '"]').show();
		...
		// If responsive reload module trigger tablet click
		if ( dslcOptionsType == 'responsive' ) {
			dslc_show_module_options_tab( DSLCString.str_res_tablet + '_responsive' );

		}

	}

	function dslc_show_module_options_tab( dslcTabID ) {

		if ( dslcDebug ) console.log( 'dslc_show_module_options_tab' );

		dslcTabID = typeof dslcTabID !== 'undefined' ? dslcTabID : false;
		...
		if ( ! dslcTabID ) {
			jQuery('.dslca-module-edit-options-tab-hook[data-section="' + dslcSectionID + '"]:first').trigger('click');
		}
	...
	function dslc_delete_module( module ) {
		...
		module.css({
			...
			module.css({ marginBottom : 0 }).animate({ height: 0 }, 800, 'easeOutQuart', function(){
				module.remove();
				dslc_generate_code();
				dslc_option_changed();
			});
	...
	function dslc_preview_change( callback ) {
		...
		if ( dslcDebug ) console.log( 'dslc_preview_change' );
	...
	function dslc_reload_module( dslcModule, callback ) {

		if ( dslcDebug ) console.log( 'dslc_reload_module' );
	...
	function dslc_cancel_changes( callback ) {
		...
		if ( dslcDebug ) console.log( 'dslc_cancel_changes' );

		callback = typeof callback !== 'undefined' ? callback : false;

		if ( jQuery('.dslca-modules-section-being-edited').length ) {
			...
			var originalOptions = jQuery('.dslca-module-options-front-backup').children().clone();
	...
	function dslc_confirm_changes( callback ) {
		...
		if ( dslcDebug ) console.log( 'dslc_confirm_changes' );
	...
	function dslc_init_colorpicker() {
		...
		if ( dslcDebug ) console.log( 'dslc_init_colorpicker' );
	...
	function dslc_init_options_scrollbar() {

		if ( dslcDebug ) console.log( 'dslc_init_options_scrollbar' );
		...
		if ( ! jQuery('body').hasClass('rtl') ) {
			...
			if ( jQuery('.dslca-module-edit-options-inner').data('jsp') ) {
				jQuery('.dslca-module-edit-options-inner').data('jsp').destroy();
			}
	...
	function dslc_init_options_slider() {
		...
		if ( dslcDebug ) console.log( 'dslc_init_options_slider' );
		...
		jQuery('.dslca-module-edit-field-slider').each(function(){
			...
			dslcSliderTooltip, dslcSliderTooltipOffset, dslcSliderHandle, dslcSliderTooltipPos, dslcModule, dslcOptionID, dslcSliderExt,
 ...
 * - dslc_delete_modules_area ( Deletes modules area )
 * - dslc_copy_modules_area ( Copies modules area )
 * - dslc_add_modules_area ( Adds a new modules area )
 */

	function dslc_modules_area_width_set( modulesArea, newWidth ) {
	...
	function dslc_delete_modules_area( modulesArea ) {

		if ( dslcDebug ) console.log( 'dslc_delete_modules_area' );

		var modulesSection = modulesArea.closest('.dslc-modules-section').find('.dslc-modules-section-inner');
		var dslcAddNew = false;

		modulesArea.addClass('dslca-modules-area-being-deleted');
		
		if ( modulesSection.find('.dslc-modules-area').length < 2 )
			dslcAddNew = true;

		if ( modulesArea.find('.dslca-module-being-edited').length ) {
	...
	function dslc_copy_modules_area( modulesArea ) {
		...
		var dslcModulesAreaCloned = modulesArea.clone().appendTo(modulesSection);

		dslcModulesAreaCloned.find('.dslca-modules-area-manage').trigger('mouseleave');
		dslcModulesAreaCloned.data('size', modulesArea.data('size') ).find('.dslc-module-front').css({ 
			'-webkit-animation-name' : 'none',
			'-moz-animation-name' : 'none',
			'animation-name' : 'none',
			'animation-duration' : '0',
	...
	function dslc_add_modules_area( dslcContainer ) {
		...
		jQuery.post(
			...
			function( response ) {
				...
				jQuery('.dslca-modules-area-loading .dslca-module-loading-inner', dslcContainer.closest('.dslc-modules-section') ).stop().animate({
					width : '100%'
				}, 200, 'linear', function(){
					dslcContainer.css({ paddingBottom : 0 });
					jQuery(this).closest('.dslca-modules-area-loading').hide();
				});
				
				setTimeout( function(){

					jQuery( response.output ).appendTo( dslcContainer ).css({ height : 0 }).animate({
						height : 99
					}, 300, function(){
		...
		// Animate loading
		var randomLoadingTime = Math.floor(Math.random() * (100 - 50 + 1) + 50) * 100;
		dslcContainer.animate({ paddingBottom : 50 }, 150);
		jQuery('.dslca-modules-area-loading', dslcContainer.closest('.dslc-modules-section') ).show();
		jQuery('.dslca-modules-area-loading .dslca-module-loading-inner', dslcContainer.closest('.dslc-modules-section') ).css({ width : 0 }).animate({
			width : '100%'
		}, randomLoadingTime, 'linear' );
 ...
 * Modules Sections
 *
 * - dslc_add_modules_section ( Add a modules section)
 * - dslc_delete_modules_section ( Deletes a modules section )
 * - dslc_edit_modules_section ( Initiates modules section editing )
 * - dslc_edit_modules_section_colorpicker
 * - dslc_copy_modules_section
 * - dslc_import_modules_section
 */

	function dslc_add_modules_section() {

		if ( dslcDebug ) console.log( 'dslc_add_modules_section' );
	...
	function dslc_delete_modules_section( dslcModulesSection ) {

		if ( dslcDebug ) console.log( 'dslc_delete_modules_section' );

		if ( dslcModulesSection.find('.dslca-module-being-edited') ) {
		...
		dslcModulesSection.trigger('mouseleave').remove();
	...
	function dslc_edit_modules_section( dslcModulesSection ) {

		if ( dslcDebug ) console.log( 'dslc_edit_modules_section' );
		...
		dslcModulesSection.addClass('dslca-modules-section-being-edited');
		...
		dslc_edit_modules_section_slider();
	...
	function dslc_edit_modules_section_colorpicker() {

		if ( dslcDebug ) console.log( 'dslc_edit_modules_section_colorpicker' );
	...
	function dslc_copy_modules_section( modulesSection ) {

		if ( dslcDebug ) console.log( 'dslc_copy_modules_section' );

		var dslcModuleID;

		var dslcModulesSectionCloned = modulesSection.clone().appendTo('#dslc-main');

		dslcModulesSectionCloned.find('.dslc-modules-area').each(function(){
			var dslcIndex = jQuery(this).index();
			jQuery(this).data('size', modulesSection.find('.dslc-modules-area:eq( ' + dslcIndex + ' )').data('size') );
		});
	...
	function dslc_edit_modules_section_slider() {

		if ( dslcDebug ) console.log( 'dslc_edit_modules_section_slider' );
	...
	function dslc_edit_modules_section_scroller() {

		if ( dslcDebug ) console.log( 'dslc_edit_modules_section_scroller' );
	...
	function dslc_import_modules_section( dslc_modules_section_code ) {	

		if ( dslcDebug ) console.log( 'dslc_import_modules_section' );
		...
		jQuery.post(
			...
			DSLCAjax.ajaxurl,
			{
				...
				dslc : 'active',
				dslc_modules_section_code : dslc_modules_section_code
			},
			function( response ) {
 ...
 * Templates
 *
 * - dslc_load_template ( Loads a template )
 * - dslc_import_template ( Import a template )
 * - dslc_save_template ( Saves a template )
 * - dslc_delete_template ( Deletes a template )
 */

	function dslc_load_template( template ) {
...
function dslc_module_edit_options_hideshow_tabs() {
	...
	if ( dslcDebug ) console.log( 'dslc_module_edit_options_hideshow_tabs' );
...
jQuery(document).ready(function($) {
	...
	$(document).on( 'click', '.dslca-module-edit-field-ttip-close', function(){
		$('.dslca-module-edit-field-ttip, .dslca-module-edit-field-icon-ttip').hide();
	});

	$(document).on( 'click', '.dslca-module-edit-field-ttip-hook', function(){

		var dslcTtip = $('.dslca-module-edit-field-ttip'),
		...
		if ( dslcTtip.is(':visible') ) {
			...
			$('.dslca-module-edit-field-ttip').hide();
			...
			$('.dslca-module-edit-field-ttip').show().css({
				top : dslcOffset.top - dslcTtipHeight - 20,
				left: dslcTtipLeft
			});

			$("head").append($('<style>.dslca-module-edit-field-ttip:after, .dslca-module-edit-field-ttip:before { left: ' + dslcTtipArrLeft + ' }</style>'));

		}

	});

	$(document).on( 'click', '.dslca-module-edit-field-icon-ttip-hook', function(){

		var dslcTtip = $('.dslca-module-edit-field-icon-ttip');
		var dslcHook = $(this);

		if ( dslcTtip.is(':visible') ) {

			$('.dslca-module-edit-field-icon-ttip').hide();
			...
			$('.dslca-module-edit-field-icon-ttip').show().css({
				top : dslcOffset.top - dslcTtipHeight - 20,
				left: dslcTtipLeft
			});

			$("head").append($('<style>.dslca-module-edit-field-icon-ttip:after, .dslca-module-edit-field-icon-ttip:before { left: ' + dslcTtipArrLeft + ' }</style>'));
	 ...
	 */
		...
		// Add Modules Section
		$(document).on( 'click', '.dslca-add-modules-section-hook', function(e){
		...
		$(document).on( 'click', '.dslca-edit-modules-section-hook', function(e){
			...
			if ( ! $(this).hasClass('dslca-action-disabled') ) {
				...
				if ( jQuery('.dslca-module-being-edited.dslca-module-change-made').length ) {
					...
					dslc_edit_modules_section( $(this).closest('.dslc-modules-section') );
		...
		$(document).on( 'click', '.dslca-copy-modules-section-hook', function(e) {

			e.preventDefault();

			if ( ! $(this).hasClass('dslca-action-disabled') ) {

				var modulesSection = $(this).closest('.dslc-modules-section');
				dslc_copy_modules_section( modulesSection );	
	...
	$(document).on( 'click', '.dslca-module-edit-field-font-next',  function(e){

		e.preventDefault();

		if ( ! $(this).hasClass('dslca-font-loading') && ! $(this).siblings('.dslca-font-loading').length ) {
	...
	$(document).on( 'click', '.dslca-module-edit-field-font-prev',  function(e){
	...
	$(document).on( 'keyup', '.dslca-module-edit-field-font', function(e) {
	...
	$(document).on( 'keypress', '.dslca-module-edit-field-font', function(e) {
	...
	/**
	 * Font Awesome Icon Change
	 */

	$(document).on( 'click', '.dslca-module-edit-field-icon-next',  function(e){
	...
	$(document).on( 'click', '.dslca-module-edit-field-icon-prev',  function(e){
	...
	$(document).on( 'keyup', '.dslca-module-edit-field-icon', function(e) {
		...
		if ( e.which != 13 && e.which != 38 && e.which != 40 ) {
			...
			dslcIconsArrayGrep = $.grep(dslcFontAwesomeArray, function(value, i) {
	...
	$(document).on( 'keypress', '.dslca-module-edit-field-icon', function(e) {
	...
	$('body').addClass('dslca-drag-not-in-progress');
	...
	$(document).on( 'click', '.dslca-prompt-modal-confirm-hook', function(e){
		...
		if (  dslcAction == 'edit_in_progress' ) {
			...
			dslc_delete_modules_section( dslcTarget.closest('.dslc-modules-section') );

		} else if ( dslcAction == 'export_modules_section' ) {

		} else if ( dslcAction == 'import_modules_section' ) {

			dslc_import_modules_section( $('.dslca-prompt-modal textarea').val() );
	...
	$(document).on( 'click', '.dslca-module-edit-options-tab-hook', function(){
		...
		dslc_show_module_options_tab( jQuery(this).data('id') );
	});
	...
	$(document).on( 'click', '.dslca-options-filter-hook', function(e){
		...
		$(this).addClass('dslca-active');
		dslc_filter_module_options( jQuery(this).data('section') );
	...
	// Preview changes on form submission
	...
	$(document).on( 'click', '.dslca-section-scroller-prev', function(e){
		...
		var scroller = $(this).closest('.dslca-section').find('.dslca-section-scroller');
		dslc_scroller_prev( scroller );
	...
	/* Text Align */
	$(document).on( 'click', '.dslca-module-edit-option-text-align-hook', function(){

		var newOpt = $(this),
		otherOpt = $(this).closest('.dslca-module-edit-option-text-align-wrapper').find('.dslca-module-edit-option-text-align-hook'),
		newVal = newOpt.data('val'),
		realOpt = $(this).closest('.dslca-module-edit-option-text-align-wrapper').siblings('input.dslca-module-edit-field');
	...
	/* Checkbox */
	$(document).on( 'click', '.dslca-module-edit-option-checkbox-hook, .dslca-modules-section-edit-option-checkbox-hook', function(){
	...
	$(document).on( 'click', '.dslca-add-modules-area-hook', function(e){
		...
		e.preventDefault();
		dslc_add_modules_area( jQuery(this).closest('.dslc-modules-section').find('.dslc-modules-section-inner') );
	...
	$(document).on( 'click', '.dslca-delete-template-hook', function(e){
		...
		e.stopPropagation();
		dslc_delete_template( $(this).data('id') );
	...
	$(document).on( 'click', '.dslca-template', function(e){

		dslc_load_template( jQuery(this).data('id') );
	...
	$(document).on( 'click', '.dslca-module-edit-hook', function(e){
		...
		if ( ! $('body').hasClass( 'dslca-composer-hidden' ) ) {
			...
			if ( $('.dslca-module-being-edited.dslca-module-change-made').length ) {
				...
				var dslcModuleID = dslcModule.data('dslc-module-id');
				var dslcModuleCurrCode = dslcModule.find('.dslca-module-code').val();

				$('.dslca-module-being-edited').removeClass('dslca-module-being-edited');
				dslcModule.addClass('dslca-module-being-edited');

				dslc_display_module_options( dslcModuleID );
	...
	$(document).on( 'keyup', '.dslca-module-edit-field-numeric', function(){
	...
	$(document).on( 'click', '.dslca-save-composer-hook', function(e){
		...
		e.preventDefault();
		if ( ! $('body').hasClass('dslca-module-saving-in-progress') ) {
			dslc_save_composer();
		}
	...
	$(document).on( 'mouseenter', '.dslca-modules-area-manage', function(){

		$(this).closest('.dslc-modules-area').addClass('dslca-options-hovered');
		...
		$(this).closest('.dslc-modules-area').removeClass('dslca-options-hovered');
	...
	$(document).on( 'mouseenter', '.dslca-drag-not-in-progress .dslc-module-front', function(e){

		 if ( ! $('body').hasClass('dslca-composer-hidden' ) ) {

			if ( $(this).height() < 190 ) 
				$('.dslca-module-manage', this).addClass('dslca-horizontal');
			else 
				$('.dslca-module-manage', this).removeClass('dslca-horizontal');

			$('.dslca-module-manage-inner', this).stop().animate({
				top: 0
			}, 300, function(){
				$(this).closest('.dslca-module-manage').css({ overflow : 'visible' });
			});

			$('.dslca-wysiwyg-actions-edit-hook', this).stop().animate({
				top: 0
			}, 300);
		...
		if ( ! $('body').hasClass('dslca-composer-hidden' ) ) {
			...
			$(this).find('.dslca-change-width-module-options').hide();
			$(this).find('.dslca-module-manage').css({ overflow : 'hidden' });
	...
	$(document).on( 'mouseenter', '.dslca-drag-not-in-progress .dslc-modules-area', function(e){

		var _this = $(this);

		 if ( ! $('body').hasClass('dslca-composer-hidden' ) ) {

		 	$('#dslc-header').addClass('dslca-header-low-z-index');

			if ( $(this).height() < 130 ) 
				$('.dslca-modules-area-manage', this).addClass('dslca-horizontal');
			else 
				$('.dslca-modules-area-manage', this).removeClass('dslca-horizontal');

			$('.dslca-modules-area-manage-inner', this).stop().animate({
				left: '0%'
			}, 300, function(){
				$( '.dslca-modules-area-manage', _this ).css({ overflow : 'visible' });
			});
		...
		if ( ! $('body').hasClass('dslca-composer-hidden' ) ) {
			...
			$('#dslc-header').removeClass('dslca-header-low-z-index');

			$( '.dslca-modules-area-manage', _this ).css({ overflow : 'hidden' });
			$('.dslca-modules-area-manage-inner', this).stop().animate({
				left: '100%'
			}, 300);
	...
	$(document).on( 'mouseenter', '.dslca-drag-not-in-progress .dslc-modules-section', function(e){

		 if ( ! $('body').hasClass('dslca-composer-hidden' ) ) {

			$('.dslca-modules-section-manage-inner', this).stop().animate({
				opacity : 1
			}, 300);
...
jQuery(document).ready(function($){
	...
	/**
	 * Prompt Modal - Confirm on Enter, Cancel on Esc
	 */
...
/**
 * Get the code for altering defaults ( DEV MODE )
 */
		...
		jQuery.post(
			...
			function( response ) {
				jQuery('.dslc-module-front:not(#' + module.attr('id') + ')[data-dslc-module-id="' + module.data('dslc-module-id') + '"][data-dslc-preset="' + module.data('dslc-preset') + '"]').each(function(){
					dslc_reload_module( jQuery(this) );
				});
			}
